...
def response_to_actions(
    ...
    # Check if we have reasoning and tool calls (for thinking models)
    if tool_calls_extracted_from_content and reasoning_stripped:
        logger.info(f'   └─ Thinking model detected: reasoning trace ({len(reasoning_stripped)} chars) + {len(tool_calls) if tool_calls else 0} tool call(s)')
